...
function App() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const web3AuthOptions: Web3AuthOptions = {
          ...
          // useAAWithExternalWallet: true, // Enables Smart Account for external wallets // No need for `useAAWithExternalWallet: true` since external wallets are not being configured
        };
        ...
        setIsInitialized(true); // Update state to indicate initialization is complete
    ...
    return () => {
      if (web3auth.current) {
        web3auth.current = null; // Set it to null to fully clean up
      }
    };
